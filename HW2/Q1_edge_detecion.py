...
if __name__ == "__main__":
    ...
    cv2.imwrite(RESULT_PATH + 'Q1-02-ver-row.jpg', abs(ver_row))

    ver_col = convolve2d(img, vx, 'valid')
    cv2.imwrite(RESULT_PATH + 'Q1-04-ver-col.jpg', abs(ver_col))

    hor_row = convolve2d(img, uy, 'valid')
    cv2.imwrite(RESULT_PATH + 'Q1-01-hor-row.jpg', abs(hor_row))

    hor_col = convolve2d(img, vy, 'valid')
    cv2.imwrite(RESULT_PATH + 'Q1-03-hor-col.jpg', abs(hor_col))

    divided_ver = convolve2d(img, convolve2d(ux, vx), 'valid')
    cv2.imwrite(RESULT_PATH + 'Q1-separated-ver.jpg', abs(divided_ver))
    cv2.imwrite(RESULT_PATH + 'Q1-scaled-separated-ver.jpg', abs(divided_ver*10))
    ver = convolve2d(img, dx, 'valid')
    print ('distance between separated and original vertical is', _get_dis(divided_ver, ver))
    cv2.imwrite(RESULT_PATH + 'Q1-06-ver.jpg', abs(ver))
    cv2.imwrite(RESULT_PATH + 'Q1-scaled-ver.jpg', abs(ver*10))

    divided_hor = convolve2d(img, convolve2d(uy, vy), 'valid')
    cv2.imwrite(RESULT_PATH + 'Q1-separated-hor.jpg', abs(divided_hor))
    cv2.imwrite(RESULT_PATH + 'Q1-scaled-separated-hor.jpg', abs(divided_hor*10))
    hor = convolve2d(img, dy, 'valid')
    print ('distance between separated and original horizontal is', _get_dis(divided_hor, hor))
    cv2.imwrite(RESULT_PATH + 'Q1-05-hor.jpg', abs(hor))
    cv2.imwrite(RESULT_PATH + 'Q1-scaled-hor.jpg', abs(hor*10))

    grad_mag = calculate_grad_mag(ver, hor)
    cv2.imwrite(RESULT_PATH + 'Q1-07-grad-mag.jpg', grad_mag)

    grad_dir = calculate_gard_dir(ver, hor)
    cv2.imwrite(RESULT_PATH + 'Q1-08-grad-dir.jpg', grad_dir)

    threshold = np.mean(grad_mag) * 3.5
    new_grad = np.zeros(grad_mag.shape)
    new_grad[grad_mag > threshold] = 255
    cv2.imwrite(RESULT_PATH + 'Q1-09-edge.jpg', new_grad)
